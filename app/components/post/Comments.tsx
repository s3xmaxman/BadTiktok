...
export default function Comments({ params }: CommentsCompTypes) {
    ...
    const addComment = async() => {
        ...
        try {
            setIsUploading(true) // アップロード中のフラグを立てる
            await useCreateComment(contextUser?.user.id, params.postId, comment) // コメントを作成する
            setCommentsByPost(params?.postId) // 投稿に関連するコメントを取得する
            setComment('') // コメントをリセットする
            setIsUploading(false) // アップロード中のフラグを解除する
        } catch (error) {
